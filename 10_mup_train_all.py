...
MUP_SWEEP_DIR = os.environ.get("MUP_SWEEP_DIR", "checkpoints/mup_lr_sweep")
RESULTS_DIR = os.environ.get("RESULTS_DIR", "checkpoints/mup_scaling")
...
def main():
    print("Stage 10: Train All μP Model Sizes")

    sweep_path = os.path.join(MUP_SWEEP_DIR, "mup_lr_sweep_summary.json")
    with open(sweep_path) as f:
        sweep = json.load(f)
    best_lr = sweep['best_lr']
    print(f"Best μP LR from sweep: {best_lr:.1e}")
    ...
    for model_name in MODEL_ORDER:
        ...
        if os.path.exists(log_path):
            print(f"\n{model_name} (μP) already trained, loading results...")
    ...
    summary_path = os.path.join(RESULTS_DIR, "mup_scaling_summary.json")
