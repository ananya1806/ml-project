...
def train_model(model, train_data, val_data, config):
    ...
    # Optimizer (nanoGPT-style parameter groups)
    ...
    # Mixed precision: bf16 on CUDA, no GradScaler needed for bf16
    use_amp = 'cuda' in device
    scaler = torch.amp.GradScaler('cuda', enabled=False)  # disabled for bf16
    ...
    for step in range(total_steps):
        # LR schedule
        lr = get_lr(step, config['learning_rate'], min_lr, warmup_steps, total_steps)
        ...
        if (step + 1) % eval_interval == 0 or step == total_steps - 1:
            ...
            # Attach val info to the latest log entry
    ...
    print(f"\n{model_name} done:val_loss={final_val_loss:.4f},"
          f"time={total_time:.1f}s,GPU={gpu_mem_gb:.1f}GB,"
          f"tok/s={tokens_processed/total_time:,.0f}")
